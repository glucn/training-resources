...
| sizes get very large and approach infinity. Pretending sample sizes and populations are infinite
| is useful for making statistical inferences and approximations since it often leads to a nice
| understanding of procedures.

...

  |==                                                                                         |   3%
| Asymptotics generally give no assurances about finite sample performance, but they form the basis
| for frequency interpretation of probabilities (the long run proportion of times an event occurs).

...

  |====                                                                                       |   4%
| Recall our simulations and discussions of sample means in previous lessons. We can now talk about
| the distribution of sample means of a collection of iid observations. The mean of the sample mean
| estimates what?
...
| It takes an integer n which is the number of coin tosses that will be simulated. As coinPlot does
| these coin flips it computes the cumulative sum (assuming heads are 1 and tails 0), but after each
| toss it divides the cumulative sum by the number of flips performed so far. It then plots this
| value for each of the k=1...n tosses. Try it now for n=10.
...
| Excellent work!
  ...
  |=========                                                                                  |  10%
| Your output depends on R's random number generator, but your plot probably jumps around a bit and,
| by the 10th flip, your cumulative sum/10 is probably different from mine. If you did this several
| times, your plots would vary quite a bit. Now call coinPlot again, this time with 10000 as the
| argument.
...
| mean of the X's, call it X', is approximately normal with mean mu and variance sigma^2/n. We
| denote this as X'~N(mu,sigma^2/n).
...
...
  ...
  |=====================                                                                      |  23%
| Notice that the CLT said nothing about the original population being normally distributed. That's
| precisely where its usefulness lies! We can assume normality of a sample mean no matter what kind
| of population we have, as long as our sample size is large enough and our samples are independent.
| Let's look at how it works with a binomial experiment like flipping a coin.

...

  |======================                                                                     |  25%
| Recall that if the probability of a head (call it 1) is p, then the probability of a tail (0) is
| 1-p. The expected value then is p and the variance is p-p^2 or p(1-p). Suppose we do n coin flips
| and let p' represent the average of these n flips. We normalize p' by subtracting the mean p and
| dividing by the std deviation sqrt(p(1-p)/n). Let's do this for 1000 trials and plot the resulting
| histogram.
...
...
  ...
  |=========================                                                                  |  27%
| Here's another plot from the slides of the same experiment, this time using a biassed coin. We set
| the probability of a head to .9, so E(X)=.9 and the standard error is sqrt(.09/n) Again, the
| larger the sample size the more closely the distribution looks normal, although with this biassed
...
| standard deviations of the mean. This figure, a standard normal with mu=0 and sigma=1, illustrates
| this point; the entire shaded portion depicts the area within 2 standard deviations of the mean
| and the darker portion shows the 68% of the area within 1 standard deviation.

...

  |=============================                                                              |  32%
| It follows that 5% of the area under the curve is not shaded. By symmetry of the curve, only 2.5%
| of the data is greater than the mean + 2 standard deviations (mu+2*sigma/sqrt(n)) and only 2.5% is
| less than the mean - 2 standard deviations (mu-2*sigma/sqrt(n)).
...
| if one were to repeatedly get samples of size n, about 95% of the intervals obtained would contain
| mu, the quantity we're trying to estimate.
...
| Excellent work!
  ...
  |============================================                                               |  48%
| Another technique for calculating confidence intervals for binomial distributions is to replace p
| with p'. This is called the Wald confidence interval. We can also use the R function qnorm to get
| a more precise quantile value (closer to 1.96) instead of our ballpark estimate of 2.

...

  |=============================================                                              |  49%
| With the formula p'+/- qnorm(.975)*sqrt(p'(1-p')/100), use the p' and n values from above and the
| R construct p'+c(-1,1)... to handle the plus/minus portion of the formula. You should see bounds
| similar to the ones you just estimated.
...
| simple null hypothesis about the probability of success in a Bernoulli experiment." (This means it
| guarantees the coverages, uses a lot of computation and doesn't rely on the CLT.) This function
| returns a lot of information but all we want now are the values of the confidence interval that it
...
| uses the sampled proportion of success, p', for those 20 coin flips to compute the upper and lower
| bounds of the 95% Wald interval, that is, it computes the two numbers p'+/- qnorm(.975) * sqrt(p'
| * (1-p') / n) for each of the 30 trials. For the given true probability p, we count the number of
| times out of those 30 trials that the true probability p was in the Wald confidence interval.
| We'll call this the coverage.

...

  |==================================================                                         |  55%
| To make sure you understand what's going on, try running mywald now with the probability .2. It
| will print out 30 p' values (which you don't really need to see), followed by 30 lower bounds, 30
| upper bounds and lastly the percentage of times that the input .2 was between the two bounds. See
| if you agree with the percentage you get. Usually it suffices to just count the number of times
| (out of the 30) that .2 is less than the upper bound.

> mywald(0.2)
[1] "Here are the p' values"
 [1] 0.15 0.30 0.25 0.25 0.05 0.10 0.15 0.25 0.20 0.20 0.25 0.50 0.10 0.35 0.35 0.20 0.30 0.25 0.15
[20] 0.25 0.20 0.20 0.30 0.35 0.10 0.20 0.30 0.25 0.15 0.20
[1] "Here are the lower"
 [1] -0.006490575  0.099163455  0.060227303  0.060227303 -0.045516829 -0.031478381 -0.006490575
 [8]  0.060227303  0.024695492  0.024695492  0.060227303  0.280869365 -0.031478381  0.140962697
[15]  0.140962697  0.024695492  0.099163455  0.060227303 -0.006490575  0.060227303  0.024695492
[22]  0.024695492  0.099163455  0.140962697 -0.031478381  0.024695492  0.099163455  0.060227303
[29] -0.006490575  0.024695492
[1] "Here are the upper"
 [1] 0.3064906 0.5008365 0.4397727 0.4397727 0.1455168 0.2314784 0.3064906 0.4397727 0.3753045
[10] 0.3753045 0.4397727 0.7191306 0.2314784 0.5590373 0.5590373 0.3753045 0.5008365 0.4397727
[19] 0.3064906 0.4397727 0.3753045 0.3753045 0.5008365 0.5590373 0.2314784 0.3753045 0.5008365
[28] 0.4397727 0.3064906 0.3753045
[1] 0.9333333
...
| probabilities, namely .1, .15, .2, .25, ... .9 . We can then plot the percentages of coverage for
...
| probability that hits close to or above the 95% line is the p=.5 . So this shows that when n, the
| number of flips, is small (20) the CLT doesn't hold for many values of p, so the Wald interval
| doesn't work very well.

...

  |======================================================                                     |  59%
| Let's try the same experiment and increase n, the number of coin flips in each of our 1000 trials,
| from 20 to 100 to see if the plot improves. Again, results may vary, but all the probabilities are
| much closer to the 95% line, so the CLT works better with a bigger value of n.
...
...
  ...
  |========================================================                                   |  62%
| Here's a plot using this Agresti/Coull interval, with 1000 trials of 20 coin flips each. It looks
| much better than both the original Wald with 20 coin flips and the improved Wald with 100 coin
| flips. However, this technique might make the confidence interval too wide.
...
...
  ...
  |===========================================================                                |  64%
| To show this simply, we wrote a function ACCompar, which takes an integer input n. For each k from
| 1 to n it computes two fractions, k/n and (k+2)/(n+4). It then prints out the boolean vector of
| whether the new (k+2)/(n+4) fraction is less than the old k/n. It also prints out the total number
| of k's for which the condition is TRUE.

...

  |============================================================                               |  66%
| For all k less than n/2, you see FALSE indicating that the new fraction is greater than or equal
| to k/n. For all k greater than n/2 you see TRUE indicating that the new fraction is less than the
| old. If k=n/2 the old and new fractions are equal.
...
| That's a job well done!
  ...
  |==============================================================                             |  68%
| Let's move on to Poisson distributions and confidence intervals. Recall that Poisson distributions
| apply to counts or rates. For the latter, we write X~Poisson(lambda*t) where lambda is the
...
| Poisson distributed with parameter (lambda*t). We don't observe the failure rate, but we estimate
| it as x/t. Call our estimate lambda_hat, so lambda_hat=x/t. According to theory, the variance of
| our estimated failure rate is lambda/t. Again, we don't observe lambda, so we use our estimate of
| it instead. We thus approximate the variance of lambda_hat as lambda_hat/t.
...
| result. This is an exact test so it guarantees coverage. As with the binomial exact test, we only
| need to look at the conf portion of the result using the x$conf construct. Do this now.
...
| experiment we ran before with binomial distributions. We'll vary our lambda values from .005 to .1
| with steps of .01 (so we have 10 of them), and for each one we'll generate 1000 Poisson samples
| with mean lambda*t. We'll calculate sample means and use them to compute 95% confidence intervals.
| We'll then count how often out of the 1000 simulations the true mean (our lambda) was contained in
...
| previous plot we used t=100 (rounding the 94.32 up). Here's a plot of the same experiment setting
| t=1000. We see that the coverage is much better for almost all the values of lambda, except for
| the smallest ones.
