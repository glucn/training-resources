...
...
  ...
  |==                                                                                         |   2%
| In this lesson, as you might expect, we'll discuss expected values. Expected values of what,
| exactly?
...
...
  ...
  |========                                                                                   |   9%
| Consider the random variable X representing a roll of a fair dice. By 'fair' we mean all the sides
| are equally likely to appear. What is the expected value of X?
...
<bytecode: 0x0000021ef48cc2c0>
<environment: 0x0000021ef33803d8>
...
...
  ...
  |=======================                                                                    |  26%
| Suppose you were rolling our two loaded dice, dice_high and dice_low. You can use this linearity
| property of expectation to compute the expected value of their average. Let X_hi and X_lo
| represent the respective outcomes of the dice roll. The expected value of the average is E((X_hi +
| X_lo)/2) or .5 *( E(X_hi)+E(X_lo) ). Compute this now. Remember we stored the expected values in
| edh and edl.
...
...
  ...
  |================================                                                           |  35%
| Here's a figure from the slides. It shows the constant (1) PDF on the left and the graph of t*f(t)
| on the right.

...

  |==================================                                                         |  37%
| Knowing that the expected value is the area under the triangle, t*f(t), what is the expected value
| of the random variable with this PDF?
...
| You are doing so well!
  ...
  |====================================                                                       |  40%
| For the purposes of illustration, here's another figure using a PDF from our previous probability
| lesson. It shows the triangular PDF f(t) on the left and the parabolic t*f(t) on the right. The
| area under the parabola between 0 and 2 represents the expected value of the random variable with
| this PDF.
...
| You got it right!
  ...
  |===============================================                                            |  51%
| Suppose spop were much bigger and we couldn't measure its mean directly and instead had to sample
| it with samples of size 2. There are 10 such samples, right? We've stored this for you in a 10 x 2
| matrix, allsam. Look at it now.
...
| mean of each row of the matrix allsam. We simply call apply with the arguments allsam, 1, and
| mean. The second argument, 1, tells 'apply' to apply the third argument 'mean' to the rows of the
| matrix. Try this now.
...
| Excellent work!
  ...
  |===================================================                                        |  56%
| You can see from the resulting vector that the sample means vary a lot, from 2.5 to 11.5, right?
| Not unexpectedly, the sample mean depends on the sample. However...

...

  |=====================================================                                      |  58%
| ... if we take the expected value of these sample means we'll see something amazing. We've stored
| the sample means in the array smeans for you. Use the R function mean on the array smeans now.
...
...
  ...
  |===========================================================                                |  65%
| Let X_1, X_2, ... X_n be a collection of n samples from a population with mean mu. The mean of
| these is (X_1 + X_2 + ... + X_n)/n.
...
...
  ...
  |===============================================================                            |  70%
| 1/n * (E(X_1) + E(X_2) + ... + E(X_n)) = (1/n)*n*mu = mu. Each E(X_i) equals mu since X_i is drawn
| from the population with mean mu. We expect, on average, a random X_i will equal mu.
...
| together. The two shaded distributions come from the same data. The blue portion represents the
| density function of randomly generated standard normal data, 100000 samples. The pink portion
| represents the density function of 10000 averages, each of 10 random normals. (The original data
| was stored in a 10000 x 10 array and the average of each row was taken to generate the pink data.)

...

  |======================================================================                     |  77%
| Here's another figure from the slides. Rolling a single die 10000 times yields the first figure.
| Each of the 6 possible outcomes appears with about the same frequency. The second figure is the
| histogram of outcomes of the average of rolling two dice. Similarly, the third figure is the
| histogram of averages of rolling three dice, and the fourth four dice. As we showed previously,
| the center or mean of the original distribution is 3.5 and that's exactly where all the panels are
| centered.

...

  |========================================================================                   |  79%
| Let's recap. Expected values are properties of distributions. The average, or mean, of random
| variables is itself a random variable and its associated distribution itself has an expected
| value. The center of this distribution is the same as that of the original distribution.
...
| Great job!
  ...
  |=======================================================================================    |  95%
| True or False? The more data that goes into the sample mean, the more concentrated its density /
| mass function is around the population mean.
